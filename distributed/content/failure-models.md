...
From the most polite failures to the most vicious — and each step to the
right costs you replicas, cryptography, or both:

```mermaid
graph LR
    CS["crash-stop<br/>dies, stays dead"] --> CR["crash-recovery<br/>dies, returns with disk state"]
    CR --> OM["omission<br/>up, drops some messages"]
    OM --> BZ["Byzantine<br/>lies, forges, colludes"]
```
...
is the model that real-world systems (databases, [Raft](#/raft), Kafka)
actually design for. It forces a discipline you'll see everywhere: **write
your critical state to disk *before* acknowledging anything to others** —
the write-ahead log. If you acknowledge first and crash before persisting,
you've made a promise your reborn self won't remember. What "on disk"
actually means here is not obvious, and [The Linux Storage
Stack](../#/storage-stack) is where the answer lives: a write that has only
reached the page cache is a write your reborn self will not find.
...
each side just believes the other is dead, and neither is lying:

```mermaid
graph LR
    subgraph I1["island 1 - believes C and D died"]
        A["A"] --- B["B"]
    end
    subgraph I2["island 2 - believes A and B died"]
        C["C"] --- D["D"]
    end
    B -. "link cut" .- C
```
...
machinery in later chapters — quorums, [consensus](#/consensus), fencing —
exists specifically to make partitions survivable. Keep this picture in
mind: it is the whole content of the "P" in [Consistency Models &
...
*suspected* dead. The entire mechanism is one timer against a stream of
messages that stopped for an unknown reason:

```mermaid
sequenceDiagram
    participant B
    participant A as A (monitor)
    B->>A: heartbeat (t=0)
    B->>A: heartbeat (t=1s)
    Note over B,A: t=2s, nothing arrives
    Note over B,A: t=3s, still nothing
    A->>A: timeout fires, suspect B dead
```

But recall the fundamental ambiguity from [The Network Is
Hostile](#/the-network-is-hostile): silence might mean B crashed, or the
network dropped the heartbeats, or B is paused (a garbage-collection pause,
a snapshot, or a [live migration](../#/live-migration) whose blackout window
freezes the guest outright — real things that stop a process for *seconds*).
The detector must pick a timeout, and:
...
hatch from the FLP impossibility theorem, which [The Consensus
Problem](#/consensus) states in full — real systems work because the network
is usually nice *enough*.
...
- **Gossip-based detection** ([CRDTs, Gossip &
  Anti-Entropy](#/crdts-and-gossip) builds the protocol): nodes share what
  they've heard about each other, so one flaky link doesn't condemn a
  healthy node.
- **Lease + fencing:** a node holds a time-limited **lease** on a role
  ...
  know it's deposed. Remember fencing: [Raft](#/raft) builds it into the
  protocol as the term number, and [Distributed
  Transactions](#/distributed-transactions) needs it to stop a deposed
  coordinator from finishing a commit. [Time, Clocks & Why They
  Lie](#/time-and-clocks) explains why the lease alone is never enough.

The same knob turns up wherever "slow" and "dead" are hard to tell apart,
and the signal you reach for is not always the honest one: [Operating
It](../inference/#/operating-it) in the inference course makes the point
that continuous batching pins a GPU's utilisation near 100% whether the
machine is starved or saturated, so the metric that looks most like a
heartbeat is the one that tells you least.
